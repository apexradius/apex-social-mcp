...
function getOrCreateKey() {
    ...
    if (fs.existsSync(KEY_FILE)) {
        ...
        throw new Error(`Encryption key file is corrupted (${raw.length} bytes, expected 32). ` +
...
function migrateLegacyKey() {
    ...
    try {
        ...
        console.error("[token-store] Could not decrypt existing tokens with legacy key. " +
...
export async function saveAccountConfig(config) {
    ...
    if (idx >= 0) {
        store.accounts[idx] = config;
    }
    else {
        store.accounts.push(config);
    }
    writeStore(store);
}
...
export async function getStoredAccount(email) {
    const store = readStore();
