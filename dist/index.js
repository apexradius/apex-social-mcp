...
import { UnifiedErrorHandler, classifyMeta, classifyGoogle, registerHealthTool, log, EXIT_CODES, } from "@apexradius/apex-mcp-shared";
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--ytdlp-path") {
            config.ytdlpPath = take("--ytdlp-path");
            continue;
        }
        if (arg === "--download-dir") {
            config.downloadDir = expandHome(take("--download-dir"));
            continue;
        }
        if (arg === "--chrome-path") {
            config.chromePath = take("--chrome-path");
            continue;
        }
        if (arg === "--no-headless") {
...
async function main() {
    ...
    registerHealthTool(server, {
        ...
        checks: {
            ...
            gmail: async () => {
                try {
                    return (await isGmailAvailable()) ? null : "No accounts or credentials";
                }
                catch (e) {
            ...
            calendar: async () => {
                try {
                    return (await isCalendarAvailable()) ? null : "No accounts or credentials";
                }
                catch (e) {
                    return e instanceof Error ? e.message : String(e);
                }
