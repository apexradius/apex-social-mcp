...
function formatEvent(event) {
    ...
    if (event.description)
        lines.push(`Description: ${String(event.description).slice(0, 500)}`);
    if (event.htmlLink)
        lines.push(`Link: ${event.htmlLink}`);
    if (event.recurringEventId)
        lines.push(`Recurring Event ID: ${event.recurringEventId}`);
    if (event.recurrence)
    ...
    if (event.reminders) {
        const rem = event.reminders;
        if (rem.useDefault) {
            lines.push('Reminders: default');
        }
        else if (rem.overrides) {
            const overrides = rem.overrides;
...
export function registerCalendarTools(server) {
    server.tool('calendar_list_events', 'List calendar events within a time range', {
        ...
        try {
            ...
            if (!events.length)
                return toolResult('No events found in the specified range.');
    ...
    server.tool('calendar_delete_event', 'Delete a calendar event (requires confirm=true)', {
        ...
        if (!confirm)
            return toolError('Safety: set confirm=true to delete the event.');
        try {
            const email = await resolveEmail();
            const cal = await getCalendarService(email);
    ...
    server.tool('calendar_get_event', 'Get full details of a specific calendar event', {
        ...
        try {
            ...
            return toolResult(formatEvent(res.data));
        }
        catch (e) {
            return toolError(e);
        }
    ...
    server.tool('calendar_free_busy', 'Check free/busy status across calendars', {
        ...
        try {
            ...
            const lines = [`Free/Busy: ${timeMin} → ${timeMax}`];
            for (const [calId, info] of Object.entries(calendars)) {
                const busyPeriods = info.busy;
    ...
    server.tool('calendar_list_calendars', 'List all calendars the user has access to', {}, async () => {
        try {
            const email = await resolveEmail();
            const cal = await getCalendarService(email);
            ...
            if (!calendars.length)
                return toolResult('No calendars found.');
    ...
    server.tool('calendar_list_recurring', 'List instances of a recurring event', {
        ...
        try {
            ...
            const res = await cal.events.instances(params);
            const instances = res.data.items ?? [];
            if (!instances.length)
                return toolResult('No instances found for this recurring event.');
            const lines = [`${instances.length} instance(s) of recurring event:`];
