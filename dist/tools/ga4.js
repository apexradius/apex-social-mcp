...
import { getGa4DataService, getGa4AdminService, resolvePropertyId, formatPropertyName, listGa4Accounts, } from '../services/ga4/client.js';
...
function parseReportResponse(data, dimensionNames, metricNames) {
...
export function registerGa4Tools(server) {
    server.tool('ga4_list_accounts', 'List configured GA4 accounts (multi-account support)', {}, async () => {
        try {
            const accounts = listGa4Accounts();
            if (!accounts.length)
                return toolResult('No GA4 accounts configured.\nSet up ~/.ga4-mcp/accounts.json or GSC_CREDENTIALS_PATH + GA4_PROPERTY_ID env vars.');
    ...
    server.tool('ga4_list_properties', 'List GA4 properties accessible by the service account', {
        ...
        try {
            ...
            if (!summaries.length)
                return toolResult('No GA4 accounts/properties found for this service account.');
    ...
    server.tool('ga4_report', 'Run a custom GA4 report with arbitrary dimensions and metrics', {
        ...
        try {
            ...
            const parsed = parseReportResponse(res.data, dimensions, metrics);
            const lines = [
    ...
    server.tool('ga4_realtime', 'Get real-time GA4 data (active users right now)', {
        ...
        try {
            ...
            const parsed = parseReportResponse(res.data, dimNames, metrics);
            const lines = [
    ...
    server.tool('ga4_user_demographics', 'Get user demographics (age, gender, geo, device)', {
        ...
        try {
            ...
            const geo = parseReportResponse(geoRes.data, ['country', 'city'], metrics);
            const device = parseReportResponse(deviceRes.data, ['deviceCategory', 'operatingSystem'], metrics);
            const age = parseReportResponse(ageRes.data, ['userAgeBracket'], metrics);
            const gender = parseReportResponse(genderRes.data, ['userGender'], metrics);
            const lines = [
    ...
    server.tool('ga4_engagement', 'Get engagement metrics (bounce rate, session duration, pages/session)', {
        ...
        try {
            ...
            const overall = parseReportResponse(overallRes.data, [], overallMetrics);
            const lines = [
            ...
            const daily = parseReportResponse(dailyRes.data, dailyDims, dailyMets);
    ...
    server.tool('ga4_compare_periods', 'Compare metrics between two date periods side by side', {
        ...
        try {
            ...
            const p1 = parseReportResponse(res1.data, [], metrics);
            const p2 = parseReportResponse(res2.data, [], metrics);
