...
export function registerGmailTools(server) {
    server.tool('gmail_list_accounts', 'List all connected Gmail accounts with status', {}, async () => {
        try {
            const accounts = await listAccounts();
            if (accounts.length === 0)
                return toolResult('No Gmail accounts connected. Run setup to add accounts.');
            const enriched = await Promise.allSettled(accounts.map(async (a) => {
                try {
    ...
    server.tool('gmail_search', 'Search for emails in a specific Gmail account', {
        ...
        try {
            ...
            if (messages.length === 0)
                return toolResult(`No results for "${query}" in ${account_email}`);
            const lines = messages.map(m => `[${m.id}] ${m.date} | From: ${m.from} | Subject: ${m.subject}`);
            return toolResult(`Found ${messages.length} messages:\n${lines.join('\n')}`);
        }
        catch (e) {
            if (isAuthError(e))
                return toolError(`Auth error for ${account_email}. Re-auth needed.`);
            return toolError(e);
        }
    });
    server.tool('gmail_search_all', 'Search for emails across ALL connected Gmail accounts', {
        ...
        try {
            const accounts = await listAccounts();
            if (accounts.length === 0)
                return toolResult('No accounts connected.');
    ...
    server.tool('gmail_read_thread', 'Read an entire email thread by thread ID', {
        ...
        try {
            ...
            const lines = [`Thread: ${thread.subject} (${thread.messages.length} messages)\n`];
...
export async function isGmailAvailable() {
    try {
