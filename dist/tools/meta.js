...
export function registerMetaTools(server, client) {
    ...
    server.tool('meta_create_post', 'Publish a text, image, or video post to Facebook Page or Instagram', {
        ...
        try {
            if (platform === 'instagram') {
                ...
                const container = await client.post(`/${client.igAccountId}/media`, body);
                const published = await client.post(`/${client.igAccountId}/media_publish`, { creation_id: container.id });
            ...
            const result = await client.pagePost(endpoint, body);
    ...
    server.tool('meta_list_posts', 'List recent posts from Facebook Page or Instagram with metrics', {
        ...
        try {
            if (platform === 'instagram') {
                const r = await client.get(`/${client.igAccountId}/media`, { fields: 'id,caption,timestamp,like_count,comments_count,media_type', limit });
            ...
            const r = await client.pageGet(`/${client.pageId}/posts`, { fields: 'id,message,created_time,likes.summary(true),comments.summary(true)', limit });
    ...
    server.tool('meta_schedule_post', 'Schedule a post for future publication', {
        ...
        try {
            const body = { message, published: false, scheduled_publish_time };
            if (image_url)
                body.url = image_url;
            const endpoint = image_url ? `/${client.pageId}/photos` : `/${client.pageId}/feed`;
            const result = await client.pagePost(endpoint, body);
    ...
    server.tool('meta_upload_media', 'Upload an image or video for use in posts or ads', {
        ...
        try {
            if (type === 'image') {
                const r = await client.postForm(`/${client.adAccountId}/adimages`, { filename: url });
            ...
            const r = await client.post(`/${client.adAccountId}/advideos`, { file_url: url });
        ...
        try {
            if (platform === 'instagram') {
                ...
                for (const img of images) {
                    const c = await client.post(`/${client.igAccountId}/media`, { image_url: img, is_carousel_item: true });
                    containerIds.push(c.id);
                }
                const carousel = await client.post(`/${client.igAccountId}/media`, { media_type: 'CAROUSEL', children: containerIds.join(','), caption });
                const pub = await client.post(`/${client.igAccountId}/media_publish`, { creation_id: carousel.id });
    ...
    server.tool('meta_list_comments', 'List comments on a Facebook or Instagram post', {
        ...
        try {
            const r = await client.pageGet(`/${post_id}/comments`, { fields: 'id,message,from,created_time', limit });
    ...
    server.tool('meta_reply_comment', 'Reply to a comment on a Facebook or Instagram post', {
        ...
        try {
            const r = await client.pagePost(`/${comment_id}/comments`, { message });
    ...
    server.tool('meta_list_messages', 'List conversations from the Facebook Page inbox (DMs)', {
        ...
        try {
            const r = await client.pageGet(`/${client.pageId}/conversations`, { fields: 'id,updated_time,snippet,participants', limit });
    ...
    server.tool('meta_page_insights', 'Get Facebook Page insights — followers, reach, impressions', {
        ...
        try {
            const r = await client.pageGet(`/${client.pageId}/insights`, { metric: metrics, period });
    ...
    server.tool('meta_post_performance', 'Get performance metrics for a specific post', {
        ...
        try {
            const r = await client.pageGet(`/${post_id}/insights`, { metric: 'post_impressions,post_engaged_users,post_reactions_by_type_total' });
            const lines = r.data.map(m => `${m.name}: ${JSON.stringify(m.values?.[0]?.value)}`);
    ...
    server.tool('meta_audience_demographics', 'Get audience demographics for Page or ad account', {
        ...
        try {
            if (source === 'page') {
                const r = await client.pageGet(`/${client.pageId}/insights`, { metric: 'page_fans_gender_age,page_fans_country,page_fans_city', period: 'lifetime' });
                const lines = r.data.map(m => `${m.name}: ${JSON.stringify(m.values?.[0]?.value)}`);
                return toolResult(lines.join('\n') || 'No demographic data.');
            }
            const r = await client.get(`/${client.adAccountId}/reachestimate`, { targeting_spec: '{}' });
    ...
    server.tool('meta_create_campaign', 'Create a new ad campaign in Meta Ads Manager', {
        ...
        try {
            ...
            const r = await client.post(`/${client.adAccountId}/campaigns`, body);
    ...
    server.tool('meta_create_adset', 'Create an ad set with targeting and budget', {
        ...
        try {
            const body = { campaign_id, name, daily_budget, billing_event: 'IMPRESSIONS', optimization_goal: 'REACH', targeting: JSON.parse(targeting), status };
            if (start_time)
                body.start_time = start_time;
            if (end_time)
                body.end_time = end_time;
            const r = await client.post(`/${client.adAccountId}/adsets`, body);
    ...
    server.tool('meta_create_ad', 'Create an ad with creative within an ad set', {
        ...
        try {
            const r = await client.post(`/${client.adAccountId}/ads`, { adset_id, name, creative: JSON.parse(creative), status });
